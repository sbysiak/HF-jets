...
def feature_importance_report(feat_importances, feat_names, print_raw=10, importance_type='', norm=True):
    ...
    def featname2objType(featname):
        if featname.startswith('Jet_SecVtx'): return 'SV'
        if featname.startswith('Jet_Track'): return 'track'
        else: return 'jet'
    ...
    if print_raw: print(f'*** Raw {importance_type}: ***')
    for i,(feat_imp, feat_name) in enumerate(sorted(zip(map(lambda x: round(x, 4), feat_importances), feat_names), reverse=True)):
        if i < print_raw: print(f'{feat_name:<55s}| {feat_imp / imp_total:.4f}')
        for k in [featname2objType(feat_name), featname2iObj(feat_name), featname2observable(feat_name)]:
            if k == None: continue
