...
def plot_roc(
    y_true, y_proba, sample_weight=None, label="", color="k", title="", ax=None
):
...
def plot_score_vs_pt(
    ...
    for pt_i in range(1, n_ptbins + 1):
        unique_flavours = list(
            set(
                [
                    "".join([i for i in s if not i.isdigit()])
                    for s in flavour_ptbin.unique()
                ]
            )
        )
...
def plot_score_vs_col(
    ...
    if ax.get_ylim()[1] < max(scores):
        ax.set_ylim(top=max(scores) + 0.2 * (max(scores) - min(scores)))
...
def plot_xgb_learning_curve(
    eval_res, metric, labels=["train set", "test set"], ax=None
):
