...
def plot_pdp(
    ...
    if scaler:
        feat_vals_orig = (
            feat_vals * np.sqrt(scaler.var_[feat_idx]) + scaler.mean_[feat_idx]
        )
    else:
    ...
    ax.set_xlim(
        left=min(ax.get_xlim()[0], min(feat_vals_orig)),
    ...
    if show_deciles:
        ...
        ax.set_xlim(xlim)
    if show_distplot:
        distplot_default_kws = dict(
            bins=np.linspace(*ax.get_xlim(), 100), distplot_y_frac=0.8
        )
        distplot_kws = {
